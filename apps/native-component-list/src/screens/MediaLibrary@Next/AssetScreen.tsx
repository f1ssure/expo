...
const AssetScreen = () => {
  ...
  const loadAssetState = async (selectedAsset: Asset) => {
    ...
    const orient = Platform.OS === 'ios' ? await selectedAsset.getOrientation() : undefined;
    const networkAsset = Platform.OS === 'ios' ? await selectedAsset.getIsInCloud() : undefined;
    const pairedVideo =
      Platform.OS === 'ios' ? await selectedAsset.getLivePhotoVideoUri() : undefined;
  ...
  const handleCheckLibraryForLivePhoto = async () => {
    ...
    try {
      ...
      for (const asset of assets) {
        ...
        if (!subtypes.includes('livePhoto')) {
          continue;
        }
